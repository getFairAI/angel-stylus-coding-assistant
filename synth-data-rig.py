...
def generate_questions_answers(text_chunks):
    messages = [
    {'role': 'system', 'content': 'You are an expert API that converts bodies of JSON with "context", "intruction" and "code" into questions and answers in JSON format. Generate up to 3 complex question and answer sets. IMMPORTANT: ONLY GENERATE QUESTION AND ANSWER SET. Your response should be an array and each JSON " \
    ...
    try:
        ...
        with open("./data/q&a_rig.jsonl", "a") as file:
...
def main():
    start_chunk = 0
    with open("./data/synth-status-rig", "r") as status:
    ...
    for i, chunk in enumerate(chunks[0:2]):
        ...
        text = f"""
            With the following context: "{chunk["metadata"]}"\n
            the expert programmer was asked to: "{chunk["instruction"]}"\n
            and generated the following code: {chunk["code"]}
        """
        
        print(text)
        start_time = time.time()
        generate_questions_answers(text)
